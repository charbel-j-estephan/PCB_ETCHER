...
def gcode_to_custom(gcode_text):
    ...
    # pcb2gcode outputs in inches, convert to mm then to 0.1mm steps
    INCH_TO_MM = 25.4
    ...
    for line in lines:
        ...
        # Pen down
        if line.startswith('G01') and 'Z' in line:
            z_match = re.search(r'Z([-\d.]+)', line)
            if z_match:
                z_val = float(z_match.group(1))
                pen_down = z_val < 0
            # Check for X Y on same line
            x_match = re.search(r'X([-\d.]+)', line)
            y_match = re.search(r'Y([-\d.]+)', line)
            if x_match: current_x = float(x_match.group(1)) * INCH_TO_MM
            if y_match: current_y = float(y_match.group(1)) * INCH_TO_MM
        ...
        # Pen up (retract)
        elif line.startswith('G00') and 'Z' in line:
            z_match = re.search(r'Z([-\d.]+)', line)
            if z_match:
                z_val = float(z_match.group(1))
                if z_val > 0:
                    ...
                    x_steps = max(0, min(1500, x_steps))
                    y_steps = max(0, min(1000, y_steps))
                    output.append(f"I{x_steps:04d}{y_steps:04d}0")

        # Movement (G00 or G01 with X Y only)
        elif line.startswith(('G00', 'G01', 'X', 'Y')):
            x_match = re.search(r'X([-\d.]+)', line)
            y_match = re.search(r'Y([-\d.]+)', line)
            if x_match: current_x = float(x_match.group(1)) * INCH_TO_MM
            if y_match: current_y = float(y_match.group(1)) * INCH_TO_MM
